<?php
  ...
  */
  ...
  $size = getimagesize('images/cat-1.jpg');
  echo "<pre>";
  print_r($size);
  echo "</pre>";
  ...
  */
  ...
  $copy = imagecreatefromjpeg('images/cat-1.jpg');
  imagejpeg($copy, 'images/cat-1-copy.jpg');
  imagedestroy($copy);
  echo "<img src=\"images/cat-1-copy.jpg\" alt=\"Cat 1 Copy\" {$size[3]}>";
  ...
  */
  ...
  $thumb = imagecreatefromjpeg('images/cat-2.jpg');
  $thumb = imagescale($thumb, 300);
  imagejpeg($thumb, 'images/cat-2-thumb.jpg');
  imagedestroy($thumb);

  $size = getimagesize('images/cat-2-thumb.jpg');
  echo "<img src=\"images/cat-2-thumb.jpg\" alt=\"Cat 2 Thumbnail\" {$size[3]} >";

  /*
  * This code creates responsive images for future use.
  */
  /* $sizes = [320, 640, 960];
  $images = ['images/cat-1.jpg'];

  for ($i = 1; $i <= 3; $i++) {
    $tmp = imagecreatefromjpeg("images/cat-{$i}.jpg");

    foreach ($sizes as $size) {
      $tmp = imagescale($tmp, $size);
      imagejpeg($tmp, "images/cat-{$i}-{$size}.jpg");
    }
    imagedestroy($tmp);
  } */
